  ...
  return (
    <section className="bg-[#0B0B0B] px-5 py-20 sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        ...
        <h2 className="ppf-heading text-[2.2rem] sm:text-[2.8rem] text-white mb-12">
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-5">
            {PROCESS.map((item) => (
              <div key={item.step} className="relative flex flex-col gap-3">
                <div className="ppf-heading w-10 h-10 rounded-full bg-[#1A1A1A] border border-[#3D3D3D] flex items-center justify-center text-[#8C8C8C] text-lg z-10">
                  {item.step}
                </div>
                <h4 className="ppf-title text-[14px] font-semibold text-white">{item.title}</h4>
                <p className="ppf-sub text-[12.5px] font-light leading-relaxed text-[#B8B8B8]">
                  {item.desc}
                </p>
              </div>
            ))}
